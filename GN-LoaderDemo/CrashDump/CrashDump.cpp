...
CrashDump::CrashDump() :m_oldExceptionFilter(NULL)
{
    ...
    if (NULL == hKernel32)
        ::OutputDebugStringW(L"[GN]:GetModuleHandle faled.\n");
    else
    {
        DWORD dwFlags = 0;
        ...
        if (procGetProcessUserModeExceptionPolicy && procSetProcessUserModeExceptionPolicy)
        {
            if (procGetProcessUserModeExceptionPolicy(&dwFlags))
            {
                ...
                ::OutputDebugStringW(format(L"[GN]:GetProcessUserModeExceptionPolicy, bRet: %lu.\n", bRet).c_str());
            }
            ::OutputDebugStringW(format(L"[GN]:SetProcessUserModeExceptionPolicy, bRet: %lu, dwFlags: %lu.\n", bRet, dwFlags).c_str());
        }
    }
}
...
LONG WINAPI CrashDump::UnhandledExceptionFilter(struct _EXCEPTION_POINTERS* pExceptionInfo)
{
    ...
    {
        ::OutputDebugStringW(L"[GN]:IsDebuggerPresent return TRUE.\n");
    ...
    {
        ::OutputDebugStringW(L"[GN]:Current function has crashed.Shit.\n");
        return EXCEPTION_CONTINUE_SEARCH;
    }
    ...
    if (0 == pathLength)
    {
        ::OutputDebugStringW(L"[GN]:GetModuleFileNameW failed.\n");
        return EXCEPTION_CONTINUE_SEARCH;
    }
    ...
    if (NULL == lastSlash)
    {
        ::OutputDebugStringW(L"[GN]:wcsrchr return wrong.\n");
        return EXCEPTION_CONTINUE_SEARCH;
    }
    ...
    { // 避免同名 
        ...
        if (::GetFileAttributes(filePath) == INVALID_FILE_ATTRIBUTES) break;
    }

    HANDLE hFile = ::CreateFileW(filePath, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
    if (INVALID_HANDLE_VALUE == hFile)
    {
        ::OutputDebugStringW(L"[GN]:CreateFileW failed.\n");
        return EXCEPTION_CONTINUE_SEARCH;
    }
    ...
    if (!hDbgHelp)
    {
        ::OutputDebugStringW(L"[GN]:LoadLibraryW DBGHELP failed.\n");
        return EXCEPTION_CONTINUE_SEARCH;
    }
    ...
    {
        ::OutputDebugStringW(L"[GN]:GetProcAddress MiniDumpWriteDump failed.\n");
    ...
    if (fnMiniDumpWriteDump(::GetCurrentProcess(), ::GetCurrentProcessId(), hFile, dumpFlags, &miniInfo, NULL, NULL))
    {
        ...
        ::swprintf_s(buffer, _countof(buffer) - 1, L"[GN]:Process has crashed.\nMinidump was saved to: \n\\%s\n", filePath);
        ::OutputDebugStringW(buffer);
        // ::MessageBoxW(NULL, buffer, NULL, MB_ICONERROR | MB_OK);
    }
    else 
    {
        ::OutputDebugStringW(format(L"[GN]:Minidump was saved failed: %hu.\n", ::GetLastError()).c_str());
